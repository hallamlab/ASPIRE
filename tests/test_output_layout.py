...
class OutputLayoutTest(unittest.TestCase):
    ...
    def test_atomic_publication_and_routing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.write(output, "preprocessing_dataset/manifest.json", "{}\n")
            ...
            self.write(staging, "community_predictor_comparison/tables/sample_grouping_model_comparison.tsv")
            self.write(staging, "community_predictor_comparison/plots/sample_grouping_model_performance.svg", "<svg/>\n")
            self.write(staging, "titan/tables/titan_taxon_results.tsv")
            self.write(staging, "titan/plots/oxygen_community_response.svg", "<svg/>\n")
            self.write(staging, "microbial_compartments/tables/microbial_compartments.tsv")
            self.write(staging, "microbial_compartments/plots/microbial_compartment_clr_pca.svg", "<svg/>\n")
            self.write(staging, "microbial_state_interpretation/tables/microbial_state_integrated_basin_comparison.tsv")
            self.write(staging, "microbial_state_interpretation/plots/hybrid_vs_microbial_compartment_curtain.svg", "<svg/>\n")
            self.write(staging, "community_turnover/tables/vertical_turnover.tsv")
            self.write(staging, "community_turnover/tables/lcbd.tsv")
            self.write(staging, "community_turnover/plots/global_lcbd_depth_time.svg", "<svg/>\n")
            self.write(staging, "group_guild_function/tables/group_guild_asv_mag_function_evidence.tsv")
            ...
            self.assertTrue((output / "modules/community_predictor_comparison/tables/sample_grouping_model_comparison.tsv").exists())
            self.assertTrue((output / "modules/community_predictor_comparison/plots/sample_grouping_model_performance.svg").exists())
            self.assertTrue((output / "modules/threshold_indicator_taxa/tables/titan_taxon_results.tsv").exists())
            self.assertTrue((output / "modules/threshold_indicator_taxa/plots/oxygen_community_response.svg").exists())
            self.assertTrue((output / "modules/microbial_compartments/tables/microbial_compartments.tsv").exists())
            self.assertTrue((output / "modules/microbial_compartments/plots/microbial_compartment_clr_pca.svg").exists())
            self.assertTrue((output / "modules/microbial_state_interpretation/tables/microbial_state_integrated_basin_comparison.tsv").exists())
            self.assertTrue((output / "modules/microbial_state_interpretation/plots/hybrid_vs_microbial_compartment_curtain.svg").exists())
            self.assertTrue((output / "modules/community_turnover/tables/vertical_turnover.tsv").exists())
            self.assertTrue((output / "modules/community_turnover/tables/lcbd.tsv").exists())
            self.assertTrue((output / "modules/community_turnover/plots/global_lcbd_depth_time.svg").exists())
            self.assertTrue((output / "modules/group_guild_function/tables/group_guild_asv_mag_function_evidence.tsv").exists())
            ...
            self.assertTrue((output / "preprocessing_dataset/manifest.json").exists())
            ...
            self.assertIn('<figure class="wide"><iframe', report)
            ...
            self.assertIn('src="../../modules/upset/plots/final_upset.svg"', report)
            self.assertIn('src="../../modules/collectors_curve/plots/collectors_curve_overlay.svg"', report)
            self.assertIn("Detailed and large analytical figures are linked here", report)
            self.assertIn('../../modules/upset/plots/">Plots</a>', report)
            self.assertIn('../../modules/collectors_curve/plots/">Plots</a>', report)
